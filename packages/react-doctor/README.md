...
itall 팀이 운영하는 [`millionco/react-doctor`](https://github.com/millionco/react-doctor) fork. React/Next.js 코드베이스의 보안·성능·정확성·접근성·번들·아키텍처·dead-code 이슈를 진단해 **0–100 점수**로 환산한다.
...
| `--dead-code`, `--no-dead-code` | unused file/export/dependency, circular import 분석 켜기/끄기     |
| `--offline`                     | deprecated no-op. itall fork는 항상 offline-only로 실행           |
...
State & Effects 2 issues
  ...
  itall-react-doctor (offline fork)
  92 / 100 Healthy
  2 issues across 1/79 files  in 479ms
```

dead-code 분석은 전체 프로젝트 reachability를 보는 검사라 `--diff` / `--staged` 모드에서는 자동으로 건너뛴다.
...
점수는 외부 API 호출 없이 로컬에서 계산한다(`packages/core`). upstream 룰, 사내 룰, dead-code 진단이 동일 파이프라인을 통해 단일 점수에 반영된다.
