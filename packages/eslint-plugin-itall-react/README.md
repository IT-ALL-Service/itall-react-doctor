...
| `itall/async-api-routes`                     | `warn`      | [async-api-routes](https://github.com/vercel-labs/agent-skills/blob/main/skills/react-best-practices/rules/async-api-routes.md)                                         |
...
### `itall/async-api-routes`

Next.js App Router의 `route.ts` / `route.tsx` 파일에서 `GET`/`POST`/`PUT`/`PATCH`/`DELETE`/`HEAD`/`OPTIONS` named export 함수 본체에 데이터 의존성 없는 `await` 두 개 이상이 연속으로 나오면 진단한다. 라우트 핸들러의 tail latency는 사용자가 직접 체감하므로 독립 호출은 `Promise.all([...])`로 race 시켜야 한다.
...
export async function GET(req: NextRequest) {
  ...
  const posts = await fetchPosts(); // user를 안 쓴다
...
```ts
export async function GET(req: NextRequest) {
  const [user, posts] = await Promise.all([fetchUser(req), fetchPosts()]);
  return Response.json({ user, posts });
}
```

파일 경로 매칭은 `context.getFilename()` 기반(`(^|/)route\.tsx?$`). 다른 파일에서는 visitor를 비워서 traversal 비용을 안 낸다. `pages/api/**`의 default export 핸들러는 upstream `server-sequential-independent-await`이 일반 케이스로 커버한다.
