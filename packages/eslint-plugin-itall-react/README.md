...
| `itall/route-segment-explicit-name`    | `warn`      | nextjs.md §4 — `page.tsx`/`layout.tsx` 등 라우팅 파일의 default export 이름은 역할이 드러나야 |
| `itall/no-document-title-mutation`     | `warn`      | nextjs.md §5 — `document.title = ...` 금지, Metadata API 사용 강제                            |
| `itall/component-function-declaration` | `warn`      | react.md — 컴포넌트는 `function` 키워드 + PascalCase                                          |
| `itall/no-type-prefix-suffix`          | `warn`      | typescript.md — `IUser`/`FooType` 같은 언어 마커 접두/접미사 금지                             |
...
### `itall/route-segment-explicit-name`

Next.js App Router 라우팅 파일(`page.tsx` / `layout.tsx` / `loading.tsx` / `not-found.tsx` / `template.tsx` / `default.tsx`)의 default export 함수 이름이 generic(`Page`, `Layout`, `Loading`, `NotFound`, `Template`, `Default`, 또는 anonymous)이면 stack trace·React DevTools·Profiler에서 어느 라우트 세그먼트인지 식별이 안 됨. 역할이 드러나는 이름으로 강제.

❌ 잘못된 예 (`app/dashboard/page.tsx`):
...
다른 라우팅 파일도 같은 패턴 (`DashboardLayout`, `DashboardLoading`, `RecruitDetailNotFound` 등). `error.tsx`는 `error-tsx-use-client` 룰이 별도로 다룬다.

### `itall/no-document-title-mutation`

`document.title = "..."` 같은 직접 변경은 크롤러/OG 미리보기 봇이 못 본다 (JS 실행 전 HTML만 읽음). App Router의 `metadata` / `generateMetadata` API로 옮겨야 초기 HTML 응답에 포함됨.

❌ 잘못된 예:

```tsx
useEffect(() => {
  document.title = `${pageTitle} | itall`;
}, [pageTitle]);
```
...
export const metadata = {
  title: "Dashboard | itall",
};
```

`AssignmentExpression`의 LHS가 `document.title` MemberExpression일 때만 발화 (computed access나 `Object.assign` 등은 의도적으로 안 잡음 — 흔한 패턴이 아님).

### `itall/component-function-declaration`

React 가이드에 따라 컴포넌트는 `function` 키워드 + PascalCase로 선언 (호이스팅으로 공개 API → 내부 헬퍼 top-down 가독성, stack trace/DevTools 라벨, TSX 제네릭 자연스러움). 컴포넌트 내부 핸들러·헬퍼는 화살표 함수 OK.
...
발화 조건 (보수적): VariableDeclarator의 id가 PascalCase Identifier이고 init이 ArrowFunctionExpression 또는 FunctionExpression이고 body가 실제로 JSX를 반환할 때. Pascal 이름이지만 JSX 안 반환하는 factory/wrapper(`const Store = () => createStore()`)는 skip.

### `itall/no-type-prefix-suffix`

`IUser`, `TFoo` 같은 언어 마커 접두사나 `UserType`, `FooInterface` 같은 마커 접미사는 TS 컴파일러가 이미 알고 있는 사실을 이름에 또 박는 것 → 노이즈. 대신 도메인 의미를 드러내는 접미사(`Model`/`Entity`/`Dto`)는 허용.
...
발화: `TSInterfaceDeclaration` / `TSTypeAliasDeclaration` 의 id가 `IUser`/`TUser` 같은 prefix 또는 `Type`/`Interface` suffix를 가질 때. `Model`/`Entity`/`Dto` 접미사로 끝나는 이름은 화이트리스트로 통과. `Program` body walk 방식이라 visitor 호환성 위험 없음.
