...
> 상태: **5/5 룰 구현 완료** — `@it-all-service/react-doctor@0.5.0` / `@it-all-service/eslint-plugin-itall-react@0.5.0` 출시 예정
...
| 5   | `itall/async-api-routes`                     | ✅ **구현 완료** (v0.5.0)                                 | route.ts HTTP method export 안의 독립 await 검출    |

PoC 5개 룰 모두 완료. 다음 우선순위는 lintability MEDIUM 카테고리 또는 운영 관측 결과를 보고 `warn`→`error` 승격 검토. 각 룰의 spec/AST 알고리즘은 §3에 보존.
...
### 룰 5: `async-api-routes` ✅ 구현 완료 (v0.5.0)
...
### ✅ Phase 4b — 남은 3개 룰 — 완료 (v0.5.0)

세 룰을 한 브랜치에 누적해 단일 PR로 묶었다.
...
20. **`async-api-routes`** — `context.getFilename()`이 `(^|/)route\.tsx?$`인 경우만 visitor 활성화. `ExportNamedDeclaration`에서 HTTP method 이름 매칭 → 본체 statement pair walk + 의존성 분석.
